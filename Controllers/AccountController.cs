...
namespace WebApplication1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private MyDbContext db = new MyDbContext(); //Ober: Add Context for UserLogin Valdation
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (model.Username.Contains("@"))
            {
                ...
                //var user = Membership.GetUserNameByEmail(model.UserName);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid && department != "" && (!model.UserName.Contains("@")))
            {
                ...
                db.DU_Relations.AddOrUpdate(
                    p => p.UserID,
                    new DU_Relation { UserID = user.Id, DepartmentID = db.Departments.Single(i => i.DepartmentName == department).Id }
                );
                db.SaveChanges();
                //Ober:end
                if (result.Succeeded)
